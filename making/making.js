...
// ==== interactive background buttons rotation (making page specific) =====
const bgButtonsContainer = document.querySelector('#catalogue-home .background-buttons');
if (bgButtonsContainer) {
  const bgButtons = bgButtonsContainer.querySelectorAll('button');
  const bgArray = Array.from(bgButtons);
  ...
  bgButtons.forEach(button => {
    ...
    button.addEventListener('click', () => {
      ...
      let randomIndex = Math.floor(Math.random() * bgArray.length);
      let randomButton = bgArray[randomIndex];

      // Make sure we don't swap with itself
      while (randomButton === button) {
        randomIndex = Math.floor(Math.random() * bgArray.length);
        randomButton = bgArray[randomIndex];
      }
