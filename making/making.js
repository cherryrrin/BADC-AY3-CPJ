...
// ==== interactive background buttons rotation (making page) =====
// Re-setup for making page to ensure all buttons work
(function() {
  const makingBgButtons = document.querySelectorAll('#catalogue-home .background-buttons button');
  const makingBgArray = Array.from(makingBgButtons);

  makingBgButtons.forEach(button => {
    let rotation = 0;
    ...
    button.addEventListener('click', () => {
      ...
      let randomIndex = Math.floor(Math.random() * makingBgArray.length);
      let randomButton = makingBgArray[randomIndex];

      // Make sure we don't swap with itself
      while (randomButton === button) {
        randomIndex = Math.floor(Math.random() * makingBgArray.length);
        randomButton = makingBgArray[randomIndex];
      }
